...
void old_ui::slider_float(float max, std::string name, float* item, std::string suffix, float step)
{
	...
	sprintf_s(value_transformed, "%0.1f", *item);

	std::stringstream ss;
	ss << value_transformed;

	if (*item > 0)
		setuped->text((*item / _pixel_value) + 10 + x_offset, y_offset + 15, setuped->visualz_font, ss.str().c_str(), Color(200, 200, 200, MenuAlpha_Text), false, 2);
