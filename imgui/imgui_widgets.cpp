...
void ui::TabButton_litt(const char* label, int* selected, int num, int total) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    ImVec2 pos = window->Pos + ImVec2(20, 100) + ImVec2(((window->Size.x - 40) / 3) * num, 0) - ImVec2(0, 1);
    ImVec2 size = ImVec2(((window->Size.x - 40) / 3), 30);
    ...
    {
        window->DrawList->AddRectFilled(pos, pos + size, ImColor(50 / 255.f, 50 / 255.f, 50 / 255.f, 255 / 255.f), 10);
    }
    ...
    if (*selected == num)
        textColor = ImColor(170 / 255.f, 170 / 255.f, 170 / 255.f, g.Style.Alpha);
...
void ui::TabButton(const char* label, int* selected, int num, int total) {
    ...
    ImVec2 pos = window->Pos + ImVec2(150, 35) + ImVec2(75 * (window->Size.x / 550) * num, 0) - ImVec2(0, 1);
    ImVec2 size = ImVec2(75 * (window->Size.x / 550), 24);
...
bool ui::Checkbox(const char* label, bool* v)
{
    ...
    if (*v)
        window->DrawList->AddRectFilled(pos + ImVec2(2, 2), pos + ImVec2(10, 10), ImColor(255, 255, 255, 255));
    else {
        if (hovered)
...
bool ui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int items)
{
    ...
    window->DrawList->AddText(cb_bb.Min + ImVec2(8, 0), ImColor(205 / 255.f, 205 / 255.f, 205 / 255.f, g.Style.Alpha), preview_value);
    window->DrawList->PopClipRect();
...
bool ui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power, int remove_from_fmt)
{
    ...
    window->DrawList->AddRectFilled(slider_bb.Min, ImVec2(slider_bb.Min.x + floorf((float)percent / 100.f * (slider_bb.Max.x - slider_bb.Min.x)), slider_bb.Max.y), ImColor(255, 255, 255, 255));
...
bool ui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (BeginPopup("picker"))
        {
            ...
            value_changed |= ColorPicker4("##picker", col, 0, &g.ColorPickerRef.x);
            EndPopup();
        }
    }
...
bool ui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    window->DrawList->PushClipRect(bb.Min, bb.Max, true);
    window->DrawList->AddText(bb.Min + ImVec2(8, size.y / 2 - label_size.y / 2 + 2), selected ? ImColor(255, 255, 255, 255) : ImColor(205 / 255.f, 205 / 255.f, 205 / 255.f, g.Style.Alpha), label);
    window->DrawList->PopClipRect();
